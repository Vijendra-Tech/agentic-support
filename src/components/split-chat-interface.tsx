...
export function SplitChatInterface({ onSendMessage }: SplitChatInterfaceProps) {
  ...
  // Add initial welcome messages when session is created
  useEffect(() => {
    if (currentSession && currentSession.messages.length === 0) {
      ...
      setTimeout(() => {
        addMessage(currentSession.id, {
          role: 'assistant',
          content: `👋 **Welcome to L2/L3 Support Assistant!**

I'm here to help you with comprehensive technical support. Here's what I can assist you with:
...
- Help with error troubleshooting
- Code review and optimization

### 📋 **Project Management**
- JIRA ticket analysis and management
...
**What would you like help with today?**`
        ...
        // Add some sample conversation examples after a delay
        setTimeout(() => {
          addMessage(currentSession.id, {
            role: 'assistant',
            content: `## 🌟 **Try These Examples**

Click on any button below to see me in action:`
          })
        }, 2000)
        
        // Add example buttons after another delay
        setTimeout(() => {
          addMessage(currentSession.id, {
            role: 'assistant',
            content: ``,
            metadata: {
              generativeUI: {
                type: 'example-buttons',
                props: {
                  examples: [
                    { text: 'Show GitHub Issues', query: 'Show me GitHub issues' },
                    { text: 'Debug Authentication', query: 'Debug the authentication issue' },
                    { text: 'Optimize Performance', query: 'Optimize performance' },
                    { text: 'Analyze JIRA Tickets', query: 'Analyze JIRA tickets' },
                    { text: 'Check Azure Pipeline', query: 'Check Azure pipeline status' }
                  ]
                }
              }
            }
  ...
  const handleSendMessage = async (message: string, metadata?: any) => {
    ...
    try {
      ...
      if (onSendMessage) {
        ...
        setTimeout(async () => {
          const response = generateMockResponse(message, metadata)
          addMessage(currentSession.id, {
            role: 'assistant',
            content: response,
            metadata: {
          ...
          // Auto-load repository files when agent suggests code changes
          await loadRelevantFiles(message, response)
          
          // Auto-open code editor if response contains code
          if (response.includes('```') && isCodeEditorCollapsed) {
  ...
  const handleOpenFile = (filename: string, content: string) => {
    ...
    // Auto-expand code editor if collapsed
    if (isCodeEditorCollapsed) {
      setIsCodeEditorCollapsed(false)
    }
  }

  // Load relevant repository files based on the conversation context
  const loadRelevantFiles = async (userMessage: string, agentResponse: string) => {
    const lowerMessage = userMessage.toLowerCase()
    const lowerResponse = agentResponse.toLowerCase()
    
    // Determine which files to load based on the conversation
    const filesToLoad: Array<{path: string, name: string}> = []
    
    if (lowerMessage.includes('auth') || lowerResponse.includes('auth') || lowerResponse.includes('login')) {
      filesToLoad.push(
        { path: 'src/auth/auth-service.ts', name: 'auth-service.ts' },
        { path: 'src/components/login-form.tsx', name: 'login-form.tsx' },
        { path: 'src/types/auth.ts', name: 'auth.ts' }
      )
    }
    
    if (lowerMessage.includes('chat') || lowerResponse.includes('chat') || lowerResponse.includes('message')) {
      filesToLoad.push(
        { path: 'src/components/chat-interface.tsx', name: 'chat-interface.tsx' },
        { path: 'src/components/chat-message.tsx', name: 'chat-message.tsx' },
        { path: 'src/components/split-chat-interface.tsx', name: 'split-chat-interface.tsx' }
      )
    }
    
    if (lowerMessage.includes('performance') || lowerResponse.includes('performance') || lowerResponse.includes('optimize')) {
      filesToLoad.push(
        { path: 'src/lib/utils.ts', name: 'utils.ts' },
        { path: 'src/hooks/use-performance.ts', name: 'use-performance.ts' },
        { path: 'package.json', name: 'package.json' }
      )
    }
    
    if (lowerMessage.includes('github') || lowerResponse.includes('github') || lowerResponse.includes('repository')) {
      filesToLoad.push(
        { path: 'src/lib/integrations.ts', name: 'integrations.ts' },
        { path: 'src/components/github-repo-drawer.tsx', name: 'github-repo-drawer.tsx' },
        { path: 'src/services/github-service.ts', name: 'github-service.ts' }
      )
    }
    
    if (lowerMessage.includes('jira') || lowerResponse.includes('jira') || lowerResponse.includes('ticket')) {
      filesToLoad.push(
        { path: 'src/services/jira-service.ts', name: 'jira-service.ts' },
        { path: 'src/components/jira-ticket-viewer.tsx', name: 'jira-ticket-viewer.tsx' }
      )
    }
    
    if (lowerMessage.includes('azure') || lowerResponse.includes('azure') || lowerResponse.includes('pipeline')) {
      filesToLoad.push(
        { path: 'azure-pipelines.yml', name: 'azure-pipelines.yml' },
        { path: 'src/services/azure-service.ts', name: 'azure-service.ts' }
      )
    }
    ...
    for (const fileInfo of filesToLoad) {
      try {
        await loadRepositoryFile(fileInfo.path, fileInfo.name)
      } catch (error) {
        console.log(`Could not load ${fileInfo.path}:`, error)
        // Create a placeholder file with example content
        createExampleFile(fileInfo.name, fileInfo.path)
      }
    }
  }
  
  // Load actual repository file
  const loadRepositoryFile = async (filePath: string, fileName: string) => {
    try {
      const response = await fetch(`/api/load-file?path=${encodeURIComponent(filePath)}`)
      if (response.ok) {
        const content = await response.text()
        handleOpenFile(fileName, content)
      } else {
        throw new Error(`Failed to load ${filePath}`)
      }
    } catch (error) {
      // Fallback to example content
      createExampleFile(fileName, filePath)
    }
  }
  
  // Create example file with realistic content
  const createExampleFile = (fileName: string, filePath: string) => {
    const exampleContent = generateExampleFileContent(fileName, filePath)
    handleOpenFile(fileName, exampleContent)
  }
